...
#                else:
        ...
        self.q_dist = Qdistribution(self.X, self.n, self.n_max, self.d, self.Kc, self.m, self.sparse, self.SS, self.area_mask, self.hyper, Z_init = self.Z_init, W_init = self.W_init, alpha_init = self.alpha_init, 
    ...
    def compute_HL(self, Y_tst, X_tst, m_in=[0], m_out=1, th=0):
        ...
        if self.method[m_out] == 0:
            ...
            if self.SS[m_out]:
                ...
                Y_tst['data'] = label_binarize(Y_tst['data'], classes = np.unique(self.d[m_out]))    
    ...
    def compute_AUC(self, Y_tst, X_tst, m_in=[0], m_out=1, th=0):
        ...
        if self.method[m_out] == 0:
            ...
            if self.SS[m_out]:
                ...
                Y_pred = self.predict(m_in, m_out, 0, X_tst)
#                Y_pred = label_binarize(np.argmax(Y_pred,axis=1), classes = np.arange(self.d[m_out]))
#        print('\n' +str( np.mean(Y_pred)))
    ...
    def update(self, Y_tst=[None], X_tst=[None], verbose=0, HL=0, AUC=0, mse=0):
        ...
        for m in np.arange(self.m):
            ...
            verboseprint('AUC W%i:       %.4f' %  (m+1, self.compute_AUC(Y_tst, X_tst)))
        self.update_Z()
        verboseprint('L(Q) Z:      %.1f' %  (self.update_bound()))
        verboseprint('AUC Z:        %.4f' %  (self.compute_AUC(Y_tst, X_tst)))
        
        for m in np.arange(self.m):
            self.update_alpha(m)
            verboseprint('L(Q) alpha%i: %.1f' %  (m+1, self.update_bound()))
            verboseprint('AUC alpha%i:   %.4f' %  (m+1, self.compute_AUC(Y_tst, X_tst)))
            #Regression
            if self.method[m] == 0:
                self.update_tau(m)
                verboseprint('L(Q) tau%i:   %.1f' %  (m+1, self.update_bound()))
                verboseprint('AUC tau%i:     %.4f' %  (m+1, self.compute_AUC(Y_tst, X_tst)))
                if self.SS[m]:
...
# =============================================================================
                    ...
                    verboseprint('AUC X%i*:      %.4f' %  (m+1, self.compute_AUC(Y_tst, X_tst)))
            #Categorical
            ...
            #MultiLabel
                ...
                for i in np.arange(2):
                    ...
                    if self.SS[m]:
                        ...
                        verboseprint ('AUC t%i*:      %.4f' %  (m+1, self.compute_AUC(Y_tst, X_tst)))
            ...
            if self.sparse[m]:
                ...
                verboseprint('AUC gamma%i:   %.4f' %  (m+1, self.compute_AUC(Y_tst, X_tst)))
    ...
    def update_Z(self):
        ...
        if not np.any(np.isnan(Z_cov)):
            ...
            print ('\nCov Z is not invertible, not updated')
...
class Qdistribution(object):
    ...
    """
    def __init__(self, X, n, n_max, d, Kc, m, sparse, SS, area_mask, hyper, Z_init=None, 
