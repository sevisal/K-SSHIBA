...
class SSHIBA(object):    
    ...
    def __init__(self, Kc = 2, prune = 0, SS_sep = 0,  hyper = None, X_init = None, 
    ...
    def fit_iterate(self, max_iter = 500, pruning_crit = 1e-6, tol = 1e-3, feat_crit = 1e-6, verbose = 0, Y_tst = [None], 
                    X_tst = [None], X_tr = [None], HL = 0, AUC = 0, ACC= 0, mse = 0, m_in = [0], m_out = 1):
...
#            print ('HL inicial:  %.4f' %  (self.compute_HL(Y_tst, X_tst)))  
        ...
        for i in range(max_iter):            
            ...
            self.update(Y_tst, X_tst, X_tr, verbose, HL, AUC, ACC, mse)
            ...
            if any(self.sparse):
                self.feature_selection(feat_crit)
            if verbose:
    ...
    def compute_predictions(self, Y_tst, X_tst, m_in=[0], m_out=1, tr=0):
        
        if None in X_tst:
            X_tst = self.X[m_in[0]]     
        if self.method[m_out] == 0:
            if self.SS[m_out]:
                Y_pred = self.X[m_out]['mean'][(self.n_max - Y_tst['data'].shape[0]):,:]#.reshape(self.n_max - self.n[m_out], self.d[m_out])
            else:
                [Y_pred, var] = self.predict(m_in, m_out, X_tst)
                
        elif self.method[m_out] == 1:
            if self.SS[m_out]:
                Y_pred = self.t[m_out]['mean'][(self.n_max - Y_tst['data'].shape[0]):,]
            else:
...
#                Y_pred = self.t[m_out]['mean'][self.SS_mask[m_out]].reshape(self.n_max - self.n[m_out], self.d[m_out])
                if tr == 1:
                    Y_pred = self.q_dist.tS[m_out]['mean'][:Y_tst['data'].shape[0],:]
                else:
                    Y_pred = self.t[m_out]['mean'][(self.n_max - Y_tst['data'].shape[0]):,:]
            else:
                Y_pred = self.predict(m_in, m_out, X_tst)
    ...
    def compute_mse(self, Y_tst, X_tst, m_in=[0], m_out=1):
        if not(type(X_tst) == dict):
            ...
            m_out = self.m-1
        Y_pred = self.compute_predictions(Y_tst, X_tst, m_in=m_in, m_out=m_out)
        if self.method[m_out] == 1:
            Y_pred = np.argmax()
    ...
    def compute_AUC(self, Y_tst, X_tst, m_in=[0], m_out=1, tr=0):
        if not(type(X_tst) == dict):
            m_in = np.arange(self.m-1)
            m_out = self.m-1
        Y_pred = self.compute_predictions(Y_tst, X_tst, m_in=m_in, m_out=m_out, tr=tr)
        if self.method[m_out] == 1:
            # Y_pred = label_binarize(Y_pred, classes = np.arange(self.d[m_out]))
            Y_tst_bin = label_binarize(Y_tst['data'], classes = np.arange(self.d[m_out])) 
        else:
            Y_tst_bin = np.copy(Y_tst['data'])
        if self.method[m_out] in {0, 2} and Y_tst['data'].shape[1] != self.d[m_out]:
    ...
    def compute_ACC(self, Y_tst, X_tst, m_in=[0], m_out=1):
        ...
        if self.method[m_out] == 2:
            Y_pred = np.argmax(Y_pred, axis=1)
            Y_real = np.argmax(Y_tst['data'], axis=1)

        return accuracy_score(Y_real, Y_pred)
    
    def update(self, Y_tst=[None], X_tst=[None], X_tr=[None], verbose=0, HL=0, AUC=0, ACC=0, mse=0, m_in=[0], m_out=1):
        """Update the variables of the model.
        ...
        for m in np.arange(self.m):  
            ...
            verboseprint ('AUC W%i:      %.4f' %  (m+1, self.compute_AUC(Y_tst, X_tst, m_in, m_out)))
        self.update_Z()
        # verboseprint('L(Q) Z:      %.1f' %  (self.update_bound()))
        verboseprint ('AUC Z:      %.4f' %  (self.compute_AUC(Y_tst, X_tst, m_in, m_out)))
        for m in np.arange(self.m):   
            self.update_alpha(m)
            # verboseprint('L(Q) alpha%i: %.1f' %  (m+1, self.update_bound()))
            verboseprint ('AUC alpha%i:      %.4f' %  (m+1, self.compute_AUC(Y_tst, X_tst, m_in, m_out)))
            #Regression
            if self.method[m] == 0:
                if self.SS[m]:
                ...
                verboseprint('AUC tau%i:     %.4f' %  (m+1, self.compute_AUC(Y_tst, X_tst, m_in, m_out)))
            ...
            # Categorical
                ...
                self.update_xcat(m)
                # verboseprint('L(Q) X%i*:     %.1f' %  (m+1, self.update_bound()))
                # verboseprint('AUC X%i:     %.4f' %  (m+1, self.compute_AUC(Y_tst, X_tst, m_in, m_out)))
                # verboseprint('ACC X%i*:       %.4f' %  (m+1, self.compute_ACC(Y_tst, X_tst, m_in, m_out)))
                if self.SS[m]:
                    self.update_tc(m)
            ...
            # MultiLabel
                ...
                for i in np.arange(2):
                    ...
                    # verboseprint('L(Q) X%i*:    %.1f' %  (m+1, self.update_bound()))
                    self.update_xi(m)
                    # verboseprint('L(Q) xi%i*:    %.1f' %  (m+1, self.update_bound()))
                    if self.SS[m]:
                        # Updating the mean and variance of t* for the SS case
                        self.update_t(m)
                        self.t[m]['mean'][self.SS_mask[m]] = q.tS[m]['mean'][self.SS_mask[m]]
                        # verboseprint('L(Q) t%i*:    %.1f' %  (m+1, self.update_bound()))
                        verboseprint ('AUC t%i*:      %.4f' %  (m+1, self.compute_AUC(Y_tst, X_tst, m_in, m_out)))
                #Update of the variable tau
                self.update_tau(m)
                # verboseprint('L(Q) tau%i*:    %.1f' %  (m+1, self.update_bound()))
                verboseprint('AUC tau%i:     %.4f' %  (m+1, self.compute_AUC(Y_tst, X_tst, m_in, m_out)))
                
            if self.method[m] == 0 or self.method[m] == 2:
                self.update_b(m)
                # verboseprint('L(Q) b%i: %.1f' %  (m+1, self.update_bound()))
                verboseprint('AUC b%i:    %.4f' %  (m+1, self.compute_AUC(Y_tst, X_tst, m_in, m_out)))
...
#                    X_tst = self.struct_data(X_tst, self.sparse[-1], self.method[-1])
            ...
            if AUC:                
                ...
                if not(None in X_tr):
                    if self.method[-1] == 0:
                        Y_tr = self.struct_data(self.X[-1]['data'], self.method[-1], self.sparse[-1])
                    else:
                        Y_tr = self.struct_data(self.t[-1]['data'], self.method[-1], self.sparse[-1])
                else:
                    X_tr = self.struct_data(self.X[0]['data'][:self.n[-1],:], self.method[0], self.sparse[0])
                    if self.method[-1] == 0:
                        Y_tr = self.struct_data(self.X[m_out]['data'], self.method[m_out], self.sparse[m_out])
                    else:
                        Y_tr = self.struct_data(self.t[m_out]['data'], self.method[m_out], self.sparse[m_out])

                self.AUC_tr.append(self.compute_AUC(Y_tr, X_tr, m_in, m_out, 1))
            if ACC:
                self.ACC.append(self.compute_ACC(Y_tst, X_tst, m_in, m_out))
                if not(None in X_tr):
                    if self.method[-1] == 0:
                        Y_tr = self.struct_data(self.X[-1]['data'][:self.n[-1],:], self.method[-1], self.sparse[-1])
                    else:
                        Y_tr = self.struct_data(self.t[-1]['data'], self.method[-1], self.sparse[-1])
                else:
                    X_tr = self.struct_data(self.X[0]['data'][:self.n[-1],:], self.method[0], self.sparse[0])
                self.ACC_tr.append(self.compute_ACC(Y_tr, X_tr, m_in, m_out))
            if mse:
        ...
        if self.sparse[m]:
            ...
            if self.area_mask[m] is not None:
                for f in np.arange(len(np.unique(self.area_mask[m]))):
                    w_cov = self.myInverse(np.diag(q.alpha_mean(m))*q.gamma_mean(m)[f] + q.tau_mean(m) * q.Z['prodT'])
                    ...
                    wwT = np.dot(q.W[m]['mean'][d,:].T, q.W[m]['mean'][d,:]) + w_cov
                    q.W[m]['prodT'] += wwT
                    DwwT = np.diag(wwT)
...
# =============================================================================
        ...
        if not np.any(np.isnan(Z_cov)):
            ...
            for (m,arg) in enumerate(args):
                if type(arg) == dict:
                    if arg['method'] == 1: #categorical
                        arg['data'] = label_binarize(arg['data'], classes = np.arange(self.d[m_in[m]]))
                    Z_mean += np.dot(arg['data'] - q.b[m]['mean'],q.W[m_in[m]]['mean']) * q.tau_mean(m_in[m])
                else:
                    for (m,x) in enumerate(arg):
                        if x['method'] == 1: #categorical
                            x['data'] = label_binarize(x['data'], classes = np.arange(self.d[m_in[m]]))
                        Z_mean += np.dot(x['data'] - q.b[m]['mean'],q.W[m_in[m]]['mean']) * q.tau_mean(m_in[m])
        ...
        if self.method[m_out] == 0:   
            #Expectation X
            mean_x = np.dot(Z_mean,q.W[m_out]['mean'].T) + q.b[m_out]['mean']
            #Variance X
...
#            p_t = np.dot(Z_mean,q.W[m_out]['mean'].T)
#            pred = np.argmax(abs(p_t),axis=1)
            
            p_t = np.zeros((n_pred,self.d[m_out]))
            m_worm = np.dot(Z_mean,q.W[m_out]['mean'].T) + q.b[m_out]['mean']
        ...
        #Multilabel
            ...
            #Expectation X
            m_x = np.dot(Z_mean, q.W[m_out]['mean'].T) + q.b[m_out]['mean']  
            #Variance X
            var_x = q.tau_mean(m_out)**(-1)*np.eye(self.d[m_out]) + np.linalg.multi_dot([q.W[m_out]['mean'], Z_cov, q.W[m_out]['mean'].T])
...
#            self.b[m]['prodT'] = self.d[m] + np.sum(self.b[m]['mean']**2)
            ...
            if method[m] == 0 or method[m] == 2:
                self.b[m]['cov'] = (1 + self.n[m] * self.tau_mean(m))**(-1) * np.eye(self.d[m])
                self.b[m]['mean'] = self.tau_mean(m) * np.dot(np.sum(X[m]['mean'] - np.dot(self.Z['mean'], W[m]['mean'].T), axis=0)[np.newaxis,:], self.b[m]['cov'])
