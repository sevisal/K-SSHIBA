    ...
    for i in np.arange(len(fold_tst)):
        ...
        if results[base]['ACC'][i] == 0.0:
            ...
            if pipeline[0] == 'KPCA':
                ...
                P_tr = cca.transform(K_tr, Y_tr_b)
                P_tst = cca.transform(K_tst, Y_tst_b)
                print('... projections defined.')
            else:
            ...
            if pipeline[1] == 'SVM':
                ...
                try:
                    results[base]['ACC'][i] = cca.score(K_tst)
                except:
            ...
            print('Fold previously trained. ' + base + ' accuracy: %0.2f%%' %(results[base]['ACC'][i]*100))
