...
def Baselines_func(folds, base, database, val=False):
    ...
    r2_tf_ss_5 = np.zeros((len(fold_tst), len(v_dim), 5))
    fl_tf_ss_5 = np.zeros((len(fold_tst), len(v_dim), 5))
                ...
                if results[base]['R2'][i, 0] == 0.0:
                    ...
                    for p, ss in enumerate(v_dim):
                        ...
                        if pipeline[0] == 'KPCA':
                            ...
                            K_tr, sig = rbf_kernel_sig(X_tr, V_ss, sig=gamma[database][0])
                            ...
                            K_tr, sig = rbf_kernel_sig(X_tr, V_ss, sig=gamma[database][2])
                            K_tst, sig = rbf_kernel_sig(X_tst, V_ss, sig = sig)
...
for database in paper_res:
    ...
    with open('kpca_kcca_test.pkl', 'wb') as output:
